...
def getBestHand(cardCounts):
    if "J" not in cardCounts:
        return "Current Hand"
    jCount = cardCounts["J"]
    if len(cardCounts) <= 2: 
        return "fiveOfAKind"
    if len(cardCounts) == 3: 
        for card in cardCounts:
            if card != "J":
                if cardCounts[card] + jCount == 4:
                    return "fourOfAKind"
        return "fullHouse"
    if len(cardCounts) == 4: 
        for card in cardCounts:
            if card != "J":
                if cardCounts[card] + jCount == 3:
                    return "threeOfAKind"
        return "twoPair"
    if len(cardCounts) == 5: 
        return "onePair"
...
def categorizeHands(hands):
    ...
    for hand in hands:
        ...
        bestHand = getBestHand(cardCounts)
        if bestHand != "Current Hand":
            categorizedHands[bestHand].append(hand)
            continue
